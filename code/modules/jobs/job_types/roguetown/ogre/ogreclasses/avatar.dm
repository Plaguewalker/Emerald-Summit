/datum/advclass/ogre/avatar
    ...
    traits_applied = list(TRAIT_STRENGTH_UNCAPPED, TRAIT_CRITICAL_RESISTANCE, TRAIT_NOPAINSTUN, TRAIT_CALTROPIMMUNE, TRAIT_STRONGBITE) //strongbite might be funny
    subclass_stats = list( 
        STATKEY_STR = 4, 
        STATKEY_CON = 5,
        STATKEY_END = 4,
...
/datum/outfit/job/roguetown/ogre/avatar/pre_equip(mob/living/carbon/human/H)
	..()
	if(H.mind)
		H.set_patron(/datum/patron/inhumen/matthios) // why would you be an avatar of graggar without following graggar
		...
		var/weaponchoice = input(H,"Choose your weapon.", "HELMET SELECTION") as anything in weapons
